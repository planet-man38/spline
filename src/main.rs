...
fn check_pressed(curve: &mut Bezier, curve_visual: Bezier, grab: &mut i8, select: &mut i8, camera: Camera) {
    ...
    if is_mouse_button_pressed(MouseButton::Left) {
        check_closest(7.0, &[camvec2(camera, curve.p0), camvec2(camera, curve.p1), camvec2(camera, curve.p2), camvec2(camera, curve.p3)], grab);
    }
    ...
    if is_mouse_button_down(MouseButton::Left) {
        if *grab >= 0 {
            ...
            *points[*grab as usize] = camvec2inv(camera, Vec2{x: mouse_position().0, y: mouse_position().1});
            }
    }
    //remove white slection thingy if mouse isn't hovering over anything
    else {
        if check_closest(7.0, &[camvec2(camera, curve.p0), camvec2(camera, curve.p1), camvec2(camera, curve.p2), camvec2(camera, curve.p3)], select) == -1 {
            *select = -1;
        }
    }
...
fn curve_bollocks(curve: &mut Bezier, grab: &mut i8, select: &mut i8, resolution: u32, edited_index: i8, index: i8, camera: Camera) {
            let drawing_curve: Bezier = cambezier(camera, *curve);
            let editing: bool;
...
fn camvec2(camera: Camera, modified: Vec2) -> Vec2 {
    (modified - camera.position) / camera.scale
}
fn camvec2inv(camera: Camera, modified: Vec2) -> Vec2 {
    (modified * camera.scale) + camera.position
}
fn cambezier(camera: Camera, modified: Bezier) -> Bezier {
    (modified - camera.position) / camera.scale
}
fn cambezierinv(camera: Camera, modified: Bezier) -> Bezier {
    (modified * camera.scale) + camera.position
}
